(() => {
  ...
  const undeliveredRecords = Array.isArray(payload.undeliveredRecords) ? payload.undeliveredRecords : [];
  ...
  const slots = {
    ...
    undelivered: document.getElementById("slot-undelivered"),
  ...
  function renderUndeliveredCard(rows, baseSentCount) {
    const total = rows.length;
    const undeliveredRate = baseSentCount ? (total / baseSentCount) * 100 : 0;
    ...
    const ordered = [...rows].sort((left, right) => String(right.data_envio).localeCompare(String(left.data_envio)));
    const tableRows = ordered.map((row) => `
      <tr>
        <td>${escapeHtml(row.cliente)}</td>
        <td>${escapeHtml(row.nome_contato)}</td>
        <td>${escapeHtml(row.csm)}</td>
        <td class="send-date-cell">${escapeHtml(formatResponseDatetime(row.data_envio))}</td>
        <td><span class="delivery-status-chip">${escapeHtml(normalizeText(row.status_processamento, "Sem status"))}</span></td>
      </tr>
    `).join("");
    ...
    return `
      <section class="card card-wide card-undelivered">
        <div class="card-head">
          ...
          <p class="chart-caption">${total} registros (${formatPercent(undeliveredRate, 2)} da base filtrável)</p>
        </div>
        <div class="${tableShellClass}"${tableShellAttrs}>
          <table>
  ...
  function renderFilterSummary(state, filteredResponses, baseSent, filteredUndelivered) {
    const chips = [];
    ...
    chips.push(`<span class="filter-chip filter-chip--soft">${filteredUndelivered.length} sem conexão WhatsApp</span>`);
  ...
  function render() {
    ...
    const filteredUndelivered = filterUndeliveredRecords(state);
    ...
    if (slots.undelivered) {
      slots.undelivered.innerHTML = renderUndeliveredCard(filteredUndelivered, baseSent.length);
    }
    applyTableScrollLimit();
    renderFilterSummary(state, filteredResponses, baseSent, filteredUndelivered);
  }
